...
const Footer = ({ setCurrentDestination }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const resortResponse = await axios.get(
          `${process.env.REACT_APP_SERVER_URL}/api/destination/getDestinations/?type=resort`
        );

        console.log("Footer data: ", resortResponse.data);
        console.log("Footer data: ", campResponse.data);
  ...
  return (
    <div>
      <Card className="bg-gray-900 rounded-none mt-32">
        <CardHeader
          floated={true}
          className="bg-white shadow-inner shadow-gray-500 border-2 border-gray-100 mb-12 mx-5 px-7 py-7 md:mx-20"
        >
          <div className="flex flex-row flex-wrap md:flex-nowrap items-center gap-y-6 gap-x-1 text-center md:justify-between justify-center bg-blue-100 rounded-lg py-5">
            <img src={logo} alt="logo-ct" className="w-72 md:w-64" />
